...
```{r gas1}
N <- 100
# sim1 <- do(10000) * summarise(resample(gas), mean(Price), .by=Competitors)
# ggplot(sim1) + geom_density(aes(x=mean.Price., fill=Competitors), alpha=.6) + theme_light()
sim2 <- do(N) * summarise(resample(gas[gas$Competitors=="Y",]), mean(Price)) # bootstrapping
...
```
This distribution represents our expectation of the average gas price charged by gas stations that have competition in sight, accounting for the inherent randomness in the sampling procedure. If we assumed that gas stations without visible competition were charging roughly the same amount, we'd expect their average to appear somewhere in the middle of this distribution. In reality, the average is represented here with a red line, falling far from the middle.

As evidenced by the plot, the average prices charged by non-competing gas stations are significantly higher than what we'd expect when accounting for natural variability. With a p value of `r sum(sim2$mean.Price. >= thresh)/10000 # p-value`, it seems likely that there is a correlation between lack of competition and higher prices.
